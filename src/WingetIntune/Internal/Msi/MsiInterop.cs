// source: https://github.com/oleg-shilo/wixsharp/blob/7601893a5e9e14408ce9ef63415290d5c56254a6/Source/src/WixSharp.Msi/WixSharp.Msi/MsiInterop.cs
...
public sealed class MsiInterop
{
    ...
    public static extern MsiError MsiEnableLog(MsiInstallLogMode mode, string file, MsiLogAttribute attributes);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiGetProductInfo(string product, string property, StringBuilder value, ref UInt32 valueSize);
    ...
    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiInstallUIHandler MsiSetExternalUI([MarshalAs(UnmanagedType.FunctionPtr)] MsiInstallUIHandler handler, MsiInstallLogMode filter, IntPtr context);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiDatabaseOpenView(IntPtr database, string query, out IntPtr view);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern IntPtr MsiCreateRecord(uint numOfFields);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiOpenDatabase(string path, MsiDbPersistMode mode, out IntPtr handle);
    ...
    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiInstallUILevel MsiSetInternalUI(MsiInstallUILevel level, ref IntPtr parentWnd);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern UInt32 MsiRecordGetFieldCount(IntPtr record);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern Int32 MsiRecordGetInteger(IntPtr record, UInt32 field);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiRecordGetString(IntPtr record, UInt32 field, StringBuilder value, ref UInt32 valueSize);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiRecordSetString(IntPtr record, uint field, string value);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiViewModify(IntPtr view, MsiModifyMode modifyMode, IntPtr record);

    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiRecordSetStream(IntPtr record, uint field, string filePath);
    ...
    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern bool MsiRecordIsNull(IntPtr record, UInt32 field);
    ...
    [DllImport("msi", CharSet = CharSet.Auto)]
    public static extern MsiError MsiGetProperty(IntPtr install, string name, StringBuilder value, ref uint valueSize);
}
